...
export default function Page() {
  ...
  // --- Calculations ---
  const fmt = (n: number) =>
    new Intl.NumberFormat(locale, { style: 'currency', currency }).format(Math.max(0, n));
  const pct = (n: number) =>
  ...
  async function generatePDF() {
    ...
    // Header
    try {
      pdf.addImage(logoSrc, 'PNG', 24, 24, 120, 32);
    } catch {}
    pdf.setFontSize(14);
      ...
      <div className="max-w-6xl mx-auto grid gap-6">
        ...
        <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-3xl md:text-4xl font-semibold tracking-tight">
                Track&Trace4Tools ROI Calculator
              </h1>
              <p className="text-slate-600 mt-1">
                Estimate annual savings from fewer plate remakes, reduced press downtime, and lower waste — in minutes.
              </p>
            </div>
          </div>
          <div className="flex flex-wrap items-center gap-2">
            ...
            <button onClick={reset} className="rounded-2xl px-4 py-2 border bg-white shadow-soft text-sm">
              Reset
            </button>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          ...
          <Stat
            ...
            value={isFinite(paybackMonths) ? `${Math.round(paybackMonths)} months` : '-'}
            subtitle="Break-even (Year 0)"
          />
        </div>
        ...
        <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
          ...
          <Card>
            ...
            <Field label="Number of presses" hint="Typical label/packaging sites run 6–12.">
              <input
                ...
                min={1}
                value={presses}
                onChange={(e) => setPresses(Number(e.target.value || 0))}
                className="input"
              />
            </Field>
            <Field label="Plate remakes per month" hint="How many plates are unnecessarily remade today?">
              <input
                type="number"
                min={0}
                value={plateRemakesPerMonth}
                onChange={(e) => setPlateRemakesPerMonth(Number(e.target.value || 0))}
                className="input"
              />
            </Field>
            <Field label="Plate-related downtime (hours/month)" hint="Estimate press downtime traced to plate/tool issues.">
              <input
                ...
                onChange={(e) => setDowntimeHoursPerMonth(Number(e.target.value || 0))}
                className="input"
              />
            </Field>
          </Card>
          ...
          <Card>
            ...
            <Field label={`Cost per plate (${currency})`} hint="Plate + handling — currency agnostic.">
              <input
                ...
                min={0}
                value={costPerPlate}
                onChange={(e) => setCostPerPlate(Number(e.target.value || 0))}
                className="input"
              />
            </Field>
            <Field label={`Downtime cost per hour (${currency})`} hint="Labor + waste + lost contribution margin.">
              <input
                ...
                onChange={(e) => setDowntimeCostPerHour(Number(e.target.value || 0))}
                className="input"
              />
            </Field>
                ...
                <div className="mt-4 space-y-4">
                  <Field label={`Material spend per month (${currency})`} hint="Used to estimate waste savings.">
                    <input
                      type="number"
                      min={0}
                      value={materialSpendPerMonth}
                      onChange={(e) => setMaterialSpendPerMonth(Number(e.target.value || 0))}
                      className="input"
                    />
                  </Field>
          ...
          <Card>
            ...
            <SliderField
              label={`Remake reduction: ${remakeReductionPct}%`}
              min={0}
              max={50}
              value={remakeReductionPct}
              setValue={setRemakeReductionPct}
              hint="Benchmark 15–25% with Track&Trace4Tools."
            />
            <SliderField
              label={`Downtime reduction: ${downtimeReductionPct}%`}
              min={0}
              max={50}
              value={downtimeReductionPct}
              setValue={setDowntimeReductionPct}
              hint="Benchmark 20–30% with proactive plate status."
            />
            <SliderField
              label={`Waste reduction: ${wasteReductionPct}%`}
              min={0}
              max={40}
              value={wasteReductionPct}
              setValue={setWasteReductionPct}
              hint="Benchmark 10–20% if waste is tracked."
            />
            ...
            <Field label={`One-time setup (${currency})`}>
              <input
                ...
                min={0}
                value={oneTimeSetup}
                onChange={(e) => setOneTimeSetup(Number(e.target.value || 0))}
                className="input"
              />
            </Field>
            <Field label={`Annual license (${currency})`}>
              <input
                ...
                min={0}
                value={annualLicense}
                onChange={(e) => setAnnualLicense(Number(e.target.value || 0))}
                className="input"
              />
            </Field>
          </Card>
        ...
        <Card>
          ...
          <p className="mt-6 text-xs text-slate-500">
            Notes: Results are indicative and depend on accurate inputs. Track&Trace4Tools improves data quality by
            tracking running meters and tool status, enabling proactive planning and fewer failures. Currency is
            display-only.
          </p>
        </Card>
        ...
        <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4 bg-white border rounded-2xl p-5 shadow-soft">
          <div>
            ...
            <div className="text-slate-600 text-sm">
              Enter your details to download a branded report with inputs & calculations you can share internally.
            </div>
          </div>
          <button
            className="rounded-2xl px-4 py-2 bg-sky-500 text-white shadow-soft"
        ...
        {/* Modal (Zoho Form embed only) */}
        {leadOpen && (
          <div className="fixed inset-0 z-50 grid place-items-center bg-black/40 p-4">
            <div className="w-full max-w-2xl rounded-2xl bg-white p-4">
              <div className="flex items-center justify-between mb-3">
                <h3 className="text-lg font-semibold">Get your Detailed PDF Report</h3>
                <button onClick={() => setLeadOpen(false)} className="px-3 py-1 rounded-lg border">
                  Close
                </button>
              </div>
              ...
              <div className="h-[70vh]">
                <iframe
                  title="TGE Lead Form"
                  // Replace with your final Zoho public URL if different:
